...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones(dim), requires_grad=True) if layer_scale_init_value > 0 else None  # 用于调整输出比例。
...
ConvNeXt
    Args:
        ...
        depths (tuple(int)): 每个阶段的 block 数量。
        dims (int): 每个阶段的维度（即特征通道数）
        drop_path_rate (float): 随机丢弃率
        layer_scale_init_value (float): 层缩放初始值。
        head_init_scale (float): 分类头的初始缩放值。
"""
class ConvNeXt(nn.Module):
    # depths和dims 表示每个阶段的残差块数量和特征通道数量  drop_path_rate：随机深度的丢弃率。
    def __init__(self, in_chans, num_classes1, num_classes2, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], drop_path_rate=0., layer_scale_init_value=1e-6, head_init_scale=1.):
        super().__init__()
        self.downsample_layers = nn.ModuleList()  # stem and 3 intermediate downsampling conv layers  存储下采样层的模块列表。
        stem = nn.Sequential(
            nn.Conv2d(in_chans, dims[0], kernel_size=(3, 2), stride=(4, 2), padding=(3, 1)),
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                ...
                nn.Conv2d(dims[i], dims[i + 1], kernel_size=(3, 2), stride=(2, 1)),
        ...
        self.head1 = nn.Linear(dims[-1], num_classes1)
        self.head2 = nn.Linear(dims[-1], num_classes2)
    ...
    def forward(self, x):
        ...
        output1 = self.head1(x)  # 第一个任务的输出
        output2 = self.head2(x)  # 第二个任务的输出
        return output1, output2  # 返回两个任务的输出
